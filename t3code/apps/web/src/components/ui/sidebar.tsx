...
const SIDEBAR_RESIZE_RESET_WIDTH = 280;
...
function SidebarRail({
  ...
  const handleDoubleClick = React.useCallback(
    (event: React.MouseEvent<HTMLButtonElement>) => {
      ...
      wrapper.style.setProperty("--sidebar-width", `${SIDEBAR_RESIZE_RESET_WIDTH}px`);
      if (resolvedResizable.storageKey && typeof window !== "undefined") {
        setLocalStorageItem(resolvedResizable.storageKey, SIDEBAR_RESIZE_RESET_WIDTH, Schema.Finite);
      }
      resolvedResizable.onResize?.(SIDEBAR_RESIZE_RESET_WIDTH);

      // Suppress the next click to avoid unwanted sidebar toggle
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "-translate-x-1/2 group-data-[side=left]:-right-4 absolute inset-y-0 z-20 hidden w-4 transition-all ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] after:rounded-full after:transition-all after:duration-150 hover:after:w-[3px] hover:after:bg-sidebar-foreground/20 group-data-[side=right]:left-0 sm:flex [[data-collapsible=offcanvas][data-state=collapsed]_&]:pointer-events-none",
