...
const readSinglePragmaValue = (db: DatabaseSync, sql: string): unknown => {
  const row = db.prepare(sql).get() as Record<string, unknown> | undefined;
  return row === undefined ? undefined : Object.values(row)[0];
};

const configureFileDatabase = (db: DatabaseSync) => {
  const journalMode = readSinglePragmaValue(db, "PRAGMA journal_mode = WAL;");
  if (String(journalMode).toLowerCase() !== "wal") {
    throw new Error(`SQLite did not enable WAL mode; journal_mode=${String(journalMode)}`);
  }
