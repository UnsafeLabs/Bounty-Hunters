...
const toRowRecord = (row: ScheduledCommandRow): ScheduledCommandRecord => {
  ...
  if (row.repeat_interval !== null) {
    return row.last_error === null
      ? { ...record, repeatInterval: row.repeat_interval }
      : { ...record, repeatInterval: row.repeat_interval, lastError: row.last_error };
  }
  return row.last_error === null ? record : { ...record, lastError: row.last_error };
};
...
export class SchedulerService {
  private readonly activeFibers = new Map<CommandId, Fiber.Fiber<unknown, unknown>>();
  ...
  private async execute(commandId: CommandId): Promise<void> {
    ...
    const retryPolicy = Schedule.exponential(Duration.seconds(1)).pipe(
      Schedule.take(record.maxRetries),
    );
