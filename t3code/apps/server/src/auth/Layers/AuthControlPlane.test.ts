...
it.layer(NodeServices.layer)("AuthControlPlane", (it) => {
  ...
  it.effect("lists sessions by most recent activity first", () =>
    Effect.gen(function* () {
      const authControlPlane = yield* AuthControlPlane;
      const sessionCredentials = yield* SessionCredentialService;
      ...
      let listed = yield* authControlPlane.listSessions();
      ...
      yield* sessionCredentials.markActive(first.sessionId);

      listed = yield* authControlPlane.listSessions();
      expect(listed.map((session) => session.sessionId)).toEqual([
        first.sessionId,
        second.sessionId,
      ]);
