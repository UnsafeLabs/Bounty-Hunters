...
describe("http request body size limits", () => {
  ...
  it("uses a 50 MB limit for upload-style routes", () => {
    expect(resolveRequestBodySizeLimit({ method: "POST", pathname: "/attachments/file" })).toBe(
      FILE_UPLOAD_REQUEST_BODY_SIZE_LIMIT_BYTES,
    );
  });
  ...
  it("matches per-route overrides by method when provided", () => {
    expect(
      resolveRequestBodySizeLimit({
        method: "PATCH",
        pathname: "/api/small",
        overrides: [{ method: "POST", pathPrefix: "/api/small", maxBytes: 128 }],
      }),
    ).toBe(DEFAULT_REQUEST_BODY_SIZE_LIMIT_BYTES);
