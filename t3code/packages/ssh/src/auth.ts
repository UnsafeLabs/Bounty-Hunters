...
const POSIX_SAFE_ASKPASS_PATH_PATTERN = /^[A-Za-z0-9_./-]+$/u;
const POSIX_SECRET_FILE_TEMPLATE = "t3code-ssh-secret.XXXXXX";
...
if [ "\${T3_SSH_AUTH_SECRET+x}" = "x" ]; then
  umask 077
  secret_file="$(mktemp "\${TMPDIR:-/tmp}/${POSIX_SECRET_FILE_TEMPLATE}")"
  cleanup() {
...
function validatePosixAskpassPath(path: string): Effect.Effect<void, SshAskpassPathError> {
  if (POSIX_SAFE_ASKPASS_PATH_PATTERN.test(path)) {
    return Effect.void;
  }
...
export const buildSshAskpassHelperDescriptor = Effect.fn(
  ...
  yield* validatePosixAskpassPath(launcherPath);
