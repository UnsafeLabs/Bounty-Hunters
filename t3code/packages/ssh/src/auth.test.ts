...
describe("ssh auth", () => {
  ...
  it.effect("creates askpass env for cached password prompts", () =>
    Effect.gen(function* () {
      ...
      assert.include(askpassScript, 'rm -f -- "$secret_file"');
  ...
  it.effect("rejects unsafe POSIX askpass paths", () =>
    Effect.sync(() => {
      ...
      for (const unsafe of [
        "/tmp/t3`whoami`/ssh-askpass.sh",
        "/tmp/t3$(whoami)/ssh-askpass.sh",
        "/tmp/t3|cat/ssh-askpass.sh",
        "/tmp/t3&cat/ssh-askpass.sh",
        "/tmp/t3<cat/ssh-askpass.sh",
        "/tmp/t3>cat/ssh-askpass.sh",
        "/tmp/t3'quote/ssh-askpass.sh",
        '/tmp/t3"quote/ssh-askpass.sh',
        "/tmp/t3\\slash/ssh-askpass.sh",
      ]) {
        assert.equal(isSafePosixAskpassPath(unsafe), false);
      }
      assert.throws(() =>
        Effect.runSync(
          buildSshAskpassHelperDescriptor({
  ...
  it.effect("builds a windows askpass launcher pair", () =>
    Effect.gen(function* () {
      ...
      assert.include(descriptor.files[1]?.contents ?? "", "Remove-Item Env:T3_SSH_AUTH_SECRET");
      assert.include(descriptor.files[1]?.contents ?? "", "ZeroFreeBSTR");
    }),
  );

  it.effect("marks POSIX askpass launcher executable only by owner", () =>
    Effect.gen(function* () {
      const descriptor = yield* buildSshAskpassHelperDescriptor({
      ...
      assert.equal(descriptor.files[0]?.mode, 0o700);
      assert.equal(descriptor.files[0]?.path, "/tmp/t3code-ssh-askpass/ssh-askpass.sh");
    }),
  );

  it("keeps askpass scripts free of tracing or log writes", () => {
    assert.notInclude(ASKPASS_POSIX_SCRIPT, "set -x");
    assert.notInclude(ASKPASS_POSIX_SCRIPT, "logger ");
    assert.notInclude(ASKPASS_WINDOWS_SCRIPT, "Write-Host");
    assert.notInclude(ASKPASS_WINDOWS_SCRIPT, "Write-Output");
  });
});
