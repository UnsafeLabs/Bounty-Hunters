...
const checks = [
  ...
  ["scheduled restart tests still pass", test.includes("restarts an unexpectedly exited backend with the Effect clock") && test.includes("does not restart after stop cancels a scheduled restart")],
  ["webview ready re-established after restart", manager.includes("yield* startHealthMonitor(runId, config.httpBaseUrl);") && manager.includes("yield* desktopWindow.handleBackendReady.pipe(")],
