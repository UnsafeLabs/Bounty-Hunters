...
test("source validates Chainlink answer, round completeness, and staleness", () => {
  ...
  assert.match(source, /require\(_fallbackFeed != address\(0\), "Invalid feed"\);/);
...
test("primary price at the exact staleness limit uses fallback", () => {
  const result = latestPrice({
    primary: { ...freshFeed, updatedAt: 6_400n },
    fallbackFeed: { ...freshFeed, price: 2_075n },
  });
  assert.deepEqual(result, { price: 2_075n, usedFallback: true });
});
