...
describe("YieldVault", function () {
  ...
  it("supports deposit, withdrawal, and reward claim flows", async () => {
    ...
    await vault.connect(distributor).notifyRewardAmount(ethers.parseEther("80"), 80n);
    ...
    assert.equal(await stakingToken.balanceOf(alice.address), ethers.parseEther("940"));

    await increaseTime(provider, 50);
    await vault.connect(alice).claimReward();
    await vault.connect(bob).claimReward();

    assert.ok((await rewardToken.balanceOf(alice.address)) > 0n);
    assert.ok((await rewardToken.balanceOf(bob.address)) > 0n);
    assert.equal(await vault.rewards(alice.address), 0n);
    assert.equal(await vault.rewards(bob.address), 0n);
  });
});
