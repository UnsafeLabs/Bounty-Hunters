...
describe("LiquidityPool", function () {
  ...
  it("removes liquidity using internal reserves so direct donations cannot be stolen", async function () {
    ...
    const amounts = await pool.connect(first).removeLiquidity.staticCall(lpBalance);

    assert.equal(amounts[0], deposit - MINIMUM_LIQUIDITY);
    assert.equal(amounts[1], deposit - MINIMUM_LIQUIDITY);
