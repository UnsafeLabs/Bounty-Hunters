...
async function deploy(factory, signer, ...args) {
  const contract = await new ethers.ContractFactory(factory.abi, factory.evm.bytecode.object, signer).deploy(...args);
...
async function setup() {
  ...
  const [owner, alice, bob, attacker] = await Promise.all([0, 1, 2, 3].map((index) => provider.getSigner(index)));
...
describe("LiquidityPool", function () {
  ...
  it("syncs internal reserves to actual balances and emits Sync", async () => {
    ...
    assert.equal(await pool.reserveA(), 1_500_000n);
    assert.equal(await pool.reserveB(), 1_250_000n);
    assert.ok(parsedLogs.some((log) => log.name === "Sync" && log.args.reserveA === 1_500_000n && log.args.reserveB === 1_250_000n));
