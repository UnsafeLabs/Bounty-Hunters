...
describe("StakingVault reentrancy hardening (#911)", function () {
  ...
  it("blocks reentrancy on withdraw via malicious contract", async function () {
    ...
    // Balance unchanged — nested reentrancy failed closed
    expect(await vault.balances(await attacker.getAddress())).to.equal(amount);
  });
  ...
  it("blocks reentrancy on claimRewards via malicious contract", async function () {
    ...
    await expect(attacker.attackClaim()).to.be.reverted;

    const vaultEthAfter = await ethers.provider.getBalance(await vault.getAddress());
    expect(vaultEthAfter).to.equal(vaultEthBefore);
  });

  it("gas delta for withdraw and claimRewards stays within +5000 vs vulnerable baseline", async function () {
    const Vuln = await ethers.getContractFactory("VulnerableStakingVault");
    const vuln = await Vuln.deploy(await token.getAddress(), rewardRate);
    await vuln.waitForDeployment();
    await user.sendTransaction({
      to: await vuln.getAddress(),
      value: ethers.parseEther("50"),
    });
    await token.connect(user).approve(await vuln.getAddress(), ethers.MaxUint256);

    const amount = ethers.parseEther("1");

    // --- withdraw gas ---
    await vault.connect(user).stake(amount);
    await vuln.connect(user).stake(amount);

    const fixedWithdraw = await (await vault.connect(user).withdraw(amount)).wait();
    const vulnWithdraw = await (await vuln.connect(user).withdraw(amount)).wait();
    const withdrawDelta = fixedWithdraw.gasUsed - vulnWithdraw.gasUsed;
    expect(withdrawDelta).to.be.lte(5000n);
    ...
    const fixedClaim = await (await vault.connect(user).claimRewards()).wait();
    const vulnClaim = await (await vuln.connect(user).claimRewards()).wait();
    const claimDelta = fixedClaim.gasUsed - vulnClaim.gasUsed;
    expect(claimDelta).to.be.lte(5000n);
  });
});
