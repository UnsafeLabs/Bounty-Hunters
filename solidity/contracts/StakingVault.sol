...
contract StakingVault is ReentrancyGuard {
    ...
    // FIX: Reentrancy — move state updates before external calls, add nonReentrant
    function withdraw(uint256 amount) external nonReentrant {
        require(balances[msg.sender] >= amount, "Insufficient balance");
        _updateReward(msg.sender);

        // FIXED: State updates before external call (checks-effects-interactions)
        balances[msg.sender] -= amount;
        totalStaked -= amount;
    ...
    // FIX: Same reentrancy pattern in claimRewards
