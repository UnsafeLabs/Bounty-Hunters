...
contract CrossChainBridge {
    ...
    constructor(address _bridgeToken, address _validator) {
        ...
        DOMAIN_SEPARATOR = keccak256(abi.encode(
            ...
            keccak256(bytes("CrossChainBridge")), keccak256(bytes("1")), block.chainid, address(this)
        ));
    }
    ...
    function processTransfer(address r, uint256 a, uint256 tn, bytes calldata s) external {
        require(tn == senderNonce[r], "Invalid nonce");
        bytes32 structHash = keccak256(abi.encode(TRANSFER_TYPEHASH, r, a, tn, block.chainid));
        bytes32 transferHash = keccak256(abi.encodePacked("\x19\x01", DOMAIN_SEPARATOR, structHash));
        require(!processedTransfers[transferHash], "Already processed");
        require(verifySignature(transferHash, s), "Invalid signature");
        processedTransfers[transferHash] = true;
        senderNonce[r]++; bridgeToken.transfer(r, a);
        emit TransferProcessed(transferHash, r, a);
    }
    function verifySignature(bytes32 hash, bytes calldata signature) public view returns (bool) {
        ...
        require(recovered != address(0), "Invalid sig: zero address");
