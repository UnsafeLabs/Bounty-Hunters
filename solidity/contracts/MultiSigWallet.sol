...
contract MultiSigWallet {
    ...
    function confirmTransaction(uint256 txId) external onlyOwner {
        ...
        confirmations[txId][msg.sender] = Confirmation(true, block.number);
    ...
    function revokeConfirmation(uint256 txId) external onlyOwner {
        ...
        confirmations[txId][msg.sender] = Confirmation(false, 0);
    ...
    function isConfirmedAtBlock(uint256 txId, uint256 blockNumber) public view returns (bool) {
        uint256 count;
        for (uint256 i = 0; i < owners.length; i++) {
            Confirmation memory c = confirmations[txId][owners[i]];
            if (c.confirmed && c.blockNumber <= blockNumber) {
                count++;
            }
        }
