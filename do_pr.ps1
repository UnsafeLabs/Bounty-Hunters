...
foreach ($issue in $issues) {
    # Ensure we are on a fresh branch from upstream/main
    git checkout main
    git fetch upstream
    git reset --hard upstream/main
    
    # Try deleting branch if it already exists from previous script run
    git branch -D fix-$issue 2>$null
    ...
    gh pr create --repo UnsafeLabs/Bounty-Hunters --head lqkhanh295:fix-$issue --title "[Antigravity] [ T3 Code ] Fix issue $issue" --body-file $prBodyFile
}
