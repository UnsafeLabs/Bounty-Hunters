...
async def websocket_endpoint(websocket: WebSocket):
    async with WebSocketWithHeartbeat(
        websocket,
        ping_interval=30,
        pong_timeout=10,
    ) as heartbeat:
...
Using the wrapper as an async context manager ensures its background heartbeat
and receive tasks are stopped when the endpoint exits.

The wrapper buffers at most 32 unconsumed application messages while monitoring
for heartbeat responses. If that backlog is exceeded, it closes the connection
with code `1013` instead of allowing unbounded memory growth.
...
`message_count` reports application messages accepted from the client
(heartbeat responses to our pings are excluded).
