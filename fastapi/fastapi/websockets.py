...
class WebSocketWithHeartbeat:
    ...
    async def _heartbeat_loop(self) -> None:
        try:
            while self._closed_at is None:
                ...
                if self._closed_at is None and self._last_pong_at < ping_sent_at:
    ...
    def _is_pong_message(self, message: Message) -> bool:
        ...
        return (
            isinstance(text, str)
            and text == self.pong_payload.decode("utf-8")
            or isinstance(data, bytes)
            and data == self.pong_payload
        )
