...
AllowOriginFunc = Callable[[str], Union[bool, Awaitable[bool]]]


class DynamicCORSMiddleware(CORSMiddleware):
    """CORS middleware with dynamic origin validation via callback.

    When ``allow_origin_func`` is provided, it is called with the request
    origin and its return value determines whether the origin is allowed.
    Both sync and async callbacks are supported. When the callback is not
    provided, origin checks fall back to the static ``allow_origins`` list
    (and optional ``allow_origin_regex``) from the base middleware.

    ``cors_max_age`` configures the ``Access-Control-Max-Age`` header on
    preflight responses. If omitted, the base ``max_age`` value is used.
    """
    ...
    def __init__(
        ...
        effective_max_age = max_age if cors_max_age is None else cors_max_age
        super().__init__(
            app=app,
        ...
        self.cors_max_age = effective_max_age
