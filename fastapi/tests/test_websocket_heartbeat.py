...
class NativePingWebSocket(FakeWebSocket):
    def __init__(self) -> None:
        super().__init__()
...
@pytest.mark.anyio
async def test_heartbeat_prefers_native_ping_when_available() -> None:
    websocket = NativePingWebSocket()
    heartbeat = WebSocketWithHeartbeat(
        websocket,
        ping_interval=0.01,
        pong_timeout=1.0,
        ping_payload=b"alive",
    )
    ...
    assert websocket.pings
    assert websocket.pings[0] == b"alive"
    assert websocket.sent_bytes == []
...
@pytest.mark.anyio
async def test_receive_json_tracks_application_messages() -> None:
    websocket = FakeWebSocket()
    heartbeat = WebSocketWithHeartbeat(websocket)
    await websocket.messages.put({"type": "websocket.receive", "text": '{"ok": true}'})

    assert await heartbeat.receive_json() == {"ok": True}
    assert heartbeat.message_count == 1


@pytest.mark.anyio
async def test_disconnect_callback_receives_close_code_and_duration_once() -> None:
    disconnects: list[tuple[int, float]] = []
    websocket = FakeWebSocket()
    heartbeat = WebSocketWithHeartbeat(
        websocket,
    ...
    duration_after_close = heartbeat.connection_duration
    await asyncio.sleep(0.01)

    assert message["type"] == "websocket.disconnect"
    assert disconnects
    assert disconnects[0][0] == 1006
    assert disconnects[0][1] == duration_after_close
    assert heartbeat.connection_duration == duration_after_close
