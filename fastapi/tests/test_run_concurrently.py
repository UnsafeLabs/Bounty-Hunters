...
@pytest.mark.anyio
async def test_run_concurrently_uses_default_limit_of_five() -> None:
    active_count = 0
    max_seen_active_count = 0
    ...
    results = await run_concurrently([tracked_task(index) for index in range(8)])

    assert results == list(range(8))
    assert max_seen_active_count == 5
...
async def test_run_concurrently_collects_all_task_exceptions() -> None:
    ...
    assert [str(exception) for exception in error.exceptions] == ["first", "second"]
...
async def test_run_concurrently_timeout_closes_unstarted_awaitables() -> None:
    ...
    with pytest.raises(ConcurrencyError) as exc_info:
        await run_concurrently(awaitables, max_concurrency=1, timeout=0.01)
