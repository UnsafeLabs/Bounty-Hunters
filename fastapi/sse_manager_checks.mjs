...
includes(source, 'disconnect_check_interval: float = 0.1', 'manager should poll for idle disconnects');
includes(source, 'asyncio.wait_for(', 'idle streams should wake periodically to detect disconnects');
...
includes(tests, 'test_manager_handles_concurrent_streams_without_blocking', 'tests should cover concurrent non-blocking streams');
includes(tests, 'test_manager_stream_cleans_up_idle_disconnects', 'tests should cover idle disconnect cleanup');
includes(tests, 'test_manager_replay_history_is_bounded', 'tests should cover bounded replay history');
