...
# Only import APIRoute for type hints; avoid circular import at runtime
...
# Compiled regex for fast sanitisation (ASCII‑safe)
_sanitise_re = re.compile(r"[^a-z0-9_]", re.ASCII)

# Global set of already generated IDs (thread‑safe)
_generated_ids: Set[str] = set()
_lock = threading.Lock()

# Safety limit to avoid infinite loops in extremely pathological cases
_MAX_SUFFIX_ATTEMPTS = 1000

# Fallback when sanitisation produces an empty string
...
def generate_unique_id(route: APIRoute) -> str:
    ...
    """
    if route is None:  # type: ignore[unreachable]
    ...
    if not methods:
        ...
        # methods is a set; we take one (consistent ordering is irrelevant)
        method = list(methods)[0].lower()

    # ---------- Extract and sanitise prefix ----------
    prefix = route.prefix or ""
    # Strip leading/trailing slashes, replace internal slashes with underscores
