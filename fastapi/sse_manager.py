...
class SSEManager:
    def __init__(self):
        self._lock = asyncio.Lock(); self._clients = {}
    ...
    async def broadcast(self, data, event="message"):
        ...
        for cid in cids:
            ...
            if c and (not c["event_types"] or event in c["event_types"]):
                try: await c["queue"].put({"event": event, "data": data}); sent += 1
                except: pass
        return sent
    ...
    async def get_connections(self, event_type=None):
        async with self._lock:
            if event_type: return [c for c, ci in self._clients.items() if event_type in ci.get("event_types", [])]
            return list(self._clients.keys())

    async def cleanup_stale(self, max_age=300):
        async with self._lock:
            now = asyncio.get_event_loop().time()
            stale = [c for c, ci in self._clients.items() if now - ci["ts"] > max_age]
            for c in stale: del self._clients[c]
