...
includes(trait, "->orderByDesc('created_at')", 'audit history should be reverse chronological by timestamp');
includes(trait, "->orderByDesc('id')", 'audit history should have deterministic tie-breaking');
...
includes(tests, 'test_audit_history_is_reverse_chronological', 'tests should cover audit history ordering');
includes(tests, 'test_audit_log_records_authenticated_user_and_request_metadata', 'tests should cover auth user and request metadata');
includes(tests, "withServerVariables(['REMOTE_ADDR' => '203.0.113.10'])", 'tests should cover request IP capture');
includes(tests, "withHeader('User-Agent', 'AuditTest/1.0')", 'tests should cover user-agent capture');
