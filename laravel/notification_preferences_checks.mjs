...
includes(controller, 'ensurePreferenceBelongsToRequester', 'single update should enforce preference ownership');
includes(controller, "->where('user_id', $userId)", 'bulk update should only update requester preferences');
includes(controller, 'abort_unless($ownedCount === count($requestedIds), 404);', 'bulk update should reject mixed-owner preference IDs');
...
includes(user, 'public function notificationPreferences(): HasMany', 'user model should expose notification preferences relationship');
...
includes(tests, 'test_users_cannot_update_another_users_preference', 'feature tests should cover single-update ownership');
includes(tests, 'test_users_cannot_bulk_update_another_users_preferences', 'feature tests should cover bulk ownership');
includes(tests, 'test_new_users_get_default_preferences_seeded', 'feature tests should cover automatic default seeding');
includes(tests, 'test_unique_constraint_prevents_duplicate_preferences', 'feature tests should cover unique preference constraint');
