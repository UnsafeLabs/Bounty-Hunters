      ...
      *================================================================
       ...
       01  WS-CURRENT-CERT.
           ...
           05  WS-ISSUER-COMMON-NAME   PIC X(256).
           05  WS-SUBJECT-COMMON-NAME  PIC X(256).
       ...
       01  WS-CERT-CHAIN.
           ...
           05  WS-CHAIN-ENTRY OCCURS 10 TIMES.
               ...
               10  WS-CHN-ISSUER       PIC X(256).
               10  WS-CHN-SUBJECT      PIC X(256).
       ...
       01  WS-AUDIT-SOURCE             PIC X(512).
       01  WS-AUDIT-PTR                PIC 9(4)  VALUE 1.
       01  WS-AUDIT-MAX-LENGTH         PIC 9(4)  VALUE 512.
       01  WS-AUDIT-FIELD-LENGTH       PIC 9(4)  VALUE 0.
       01  WS-AUDIT-SPACE-LEFT         PIC 9(4)  VALUE 0.
       01  WS-AUDIT-COPY-LENGTH        PIC 9(4)  VALUE 0.
       01  WS-AUDIT-MARKER-START       PIC 9(4)  VALUE 0.
       01  WS-AUDIT-DELIMITER          PIC X     VALUE '|'.
       01  WS-AUDIT-TRUNCATED-MARKER   PIC X(11)
           VALUE '[TRUNCATED]'.
       01  WS-AUDIT-TRUNCATED-FLAG     PIC X     VALUE 'N'.
           88  WS-AUDIT-TRUNCATED      VALUE 'Y'.
       ...
       8000-WRITE-AUDIT-ENTRY.
           ...
           MOVE FUNCTION TRIM(WS-AUDIT-TIMESTAMP)
               TO WS-AUDIT-SOURCE
           PERFORM 8100-APPEND-AUDIT-FIELD
           MOVE WS-AUDIT-DELIMITER TO WS-AUDIT-SOURCE
           PERFORM 8100-APPEND-AUDIT-FIELD
           MOVE FUNCTION TRIM(WS-CERT-SERIAL-NUM)
               TO WS-AUDIT-SOURCE
           PERFORM 8100-APPEND-AUDIT-FIELD
           MOVE WS-AUDIT-DELIMITER TO WS-AUDIT-SOURCE
           PERFORM 8100-APPEND-AUDIT-FIELD
           MOVE FUNCTION TRIM(WS-ISSUER-COMMON-NAME)
               TO WS-AUDIT-SOURCE
           PERFORM 8100-APPEND-AUDIT-FIELD
           MOVE WS-AUDIT-DELIMITER TO WS-AUDIT-SOURCE
           PERFORM 8100-APPEND-AUDIT-FIELD
           MOVE FUNCTION TRIM(WS-SUBJECT-COMMON-NAME)
               TO WS-AUDIT-SOURCE
           PERFORM 8100-APPEND-AUDIT-FIELD
           MOVE WS-AUDIT-DELIMITER TO WS-AUDIT-SOURCE
           PERFORM 8100-APPEND-AUDIT-FIELD
           MOVE WS-VALIDATION-RESULT TO WS-AUDIT-SOURCE
           PERFORM 8100-APPEND-AUDIT-FIELD
           MOVE WS-AUDIT-DELIMITER TO WS-AUDIT-SOURCE
           PERFORM 8100-APPEND-AUDIT-FIELD
           MOVE FUNCTION TRIM(WS-VALIDATION-MSG)
               TO WS-AUDIT-SOURCE
           PERFORM 8100-APPEND-AUDIT-FIELD
       ...
       8100-APPEND-AUDIT-FIELD.
           ...
           IF WS-AUDIT-PTR > WS-AUDIT-MAX-LENGTH
               ...
               GO TO 8100-EXIT
           END-IF
           COMPUTE WS-AUDIT-FIELD-LENGTH =
               FUNCTION LENGTH(FUNCTION TRIM(WS-AUDIT-SOURCE))
           COMPUTE WS-AUDIT-SPACE-LEFT =
               WS-AUDIT-MAX-LENGTH - WS-AUDIT-PTR + 1
           IF WS-AUDIT-FIELD-LENGTH > WS-AUDIT-SPACE-LEFT
               IF WS-AUDIT-SPACE-LEFT >
                   FUNCTION LENGTH(WS-AUDIT-TRUNCATED-MARKER)
                   COMPUTE WS-AUDIT-COPY-LENGTH =
                       WS-AUDIT-SPACE-LEFT -
                       FUNCTION LENGTH(WS-AUDIT-TRUNCATED-MARKER)
                   STRING
                       WS-AUDIT-SOURCE(1:WS-AUDIT-COPY-LENGTH)
                           DELIMITED BY SIZE
                       WS-AUDIT-TRUNCATED-MARKER DELIMITED BY SIZE
           ...
           END-IF
           STRING
               WS-AUDIT-SOURCE(1:WS-AUDIT-FIELD-LENGTH)
                   DELIMITED BY SIZE
               INTO WS-AUDIT-RECORD
               WITH POINTER WS-AUDIT-PTR
               ON OVERFLOW
       ...
       8200-MARK-AUDIT-TRUNCATED.
           IF WS-AUDIT-NOT-TRUNCATED
               ...
               SET WS-AUDIT-TRUNCATED TO TRUE
           END-IF
           COMPUTE WS-AUDIT-MARKER-START =
               WS-AUDIT-MAX-LENGTH -
               FUNCTION LENGTH(WS-AUDIT-TRUNCATED-MARKER) + 1
           MOVE WS-AUDIT-TRUNCATED-MARKER
               TO WS-AUDIT-RECORD
                   (WS-AUDIT-MARKER-START:
                    FUNCTION LENGTH(WS-AUDIT-TRUNCATED-MARKER))
           COMPUTE WS-AUDIT-PTR = WS-AUDIT-MAX-LENGTH + 1
           .
       9000-CLEANUP.
