...
impl std::fmt::Display for SessionError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            SessionError::CachePoisoned => write!(f, "session cache lock is poisoned"),
...
mod tests {
    ...
    fn ticket(ticket_id: String) -> SessionTicket {
        let now = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap_or(Duration::ZERO)
            .as_secs();
        ...
        SessionTicket {
            ...
            issued_at: now,
            lifetime_secs: DEFAULT_TICKET_LIFETIME_SECS,
            encrypted_state: vec![0; 16],
            creation_time: now,
        }
    }

    #[test]
    fn concurrent_store_and_get_session() {
        ...
        let handles: Vec<_> = (0..10)
            .map(|i| {
                ...
                thread::spawn(move || {
                    let ticket_id = format!("tkt_1_{}", i);
                    cache.store_session(ticket(ticket_id.clone())).unwrap();

                    let stored = cache.get_session(&ticket_id).unwrap();
